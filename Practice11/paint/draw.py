...
drawing = False
start_pos = None
last_pos = None          # Store previous mouse position for continuous line
...
def draw_line_between(pos1, pos2, color, size):
    """Draw continuous line between two points (fixes dotted line problem)"""
    x1, y1 = pos1
    x2, y2 = pos2
    ...
    if distance == 0:
        pygame.draw.circle(canvas, color, pos1, size)
    else:
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if mouse_y > SCREEN_HEIGHT - TOOLBAR_HEIGHT:
                ...
                last_pos = (mouse_x, mouse_y)  # Store first position for continuous line
            ...
            if mouse_y < SCREEN_HEIGHT - TOOLBAR_HEIGHT:
                if current_tool == "brush":
                    ...
                    if last_pos:
                        draw_line_between(last_pos, (mouse_x, mouse_y), current_color, brush_size)
                    draw_brush((mouse_x, mouse_y), current_color, brush_size)
                    last_pos = (mouse_x, mouse_y)  # Update last position
                elif current_tool == "eraser":
                    # Same fix for eraser
                    if last_pos:
                        draw_line_between(last_pos, (mouse_x, mouse_y), WHITE, brush_size)
                    draw_eraser((mouse_x, mouse_y), brush_size)
                    last_pos = (mouse_x, mouse_y)  # Update last position
